...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [c, s, t, d, p, lt, yl, gt] = await Promise.all([
        ...
        loadKey(K_SETTINGS, { cardTime: "08:00", scoreTime: "21:00", musicStyle: null, notifyEnabled: false }),
        loadKey(K_TODOS, []),
        loadKey(K_DAILY, {}),
        loadKey(K_POSTCARDS, []),
        loadKey(K_LETTERS, {}),
  ...
  /* 背景音樂：柔和鋼琴。只有在「創帳號完成」且「使用者選了要音樂」時才會播。
     創角過程中完全不啟動任何音樂。第一次碰畫面才會出聲（瀏覽器規定）。 */
  const musicEnabled = character.onboarded && settings.musicStyle === "piano";
  ...
  return (
    <div id="app-root" className="fixed inset-0 flex flex-col app-bg-gradient text-main font-sans overflow-hidden">
      ...
      {!showSplash && (settings.musicStyle === null || settings.musicStyle === undefined) && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black-o40 modal-blur px-6">
          <div className="bg-paper rounded-3xl p-6 pt-7 w-full max-w-xs text-center shadow-xl border border-line" style={{ animation: "cheerPop .4s cubic-bezier(.2,.7,.3,1)" }}>
            ...
            <div className="flex flex-col gap-2">
              <button
                onClick={() => { setSettings((s) => ({ ...s, musicStyle: "piano" })); musicEngine.play("piano"); }}
                className="w-full bg-ink text-paper rounded-full py-3 text-sm"
              >好，播放柔和鋼琴</button>
              <button
                onClick={() => setSettings((s) => ({ ...s, musicStyle: "off" }))}
