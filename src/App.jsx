...
const musicEngine = {
  nodes: [],
  current: null,
  token: 0,          // 每次 play 遞增；async 期間若被新呼叫取代就放棄
  disposing: false,
  dispose() {
    this.disposing = true;
    // 先讓所有發聲節點立刻靜音並斷開，聲音馬上停（不等 release / reverb 尾音）
    this.nodes.forEach((n) => {
      try { if (n.volume) n.volume.value = -Infinity; } catch {}
      try { if (typeof n.releaseAll === "function") n.releaseAll(); } catch {}
  ...
  async play(style) {
    // 取一個新號碼；只要之後又被呼叫，這一次就作廢，避免 async 期間兩軌疊起來
    const myToken = ++this.token;
    if (style === this.current) return;
    this.dispose();
    if (!style || style === "off") return;
    await Tone.start();
    if (myToken !== this.token) return; // 期間又被呼叫 → 放棄，交給最新那次
    ...
    // 建好後再確認一次沒被取代
    if (myToken !== this.token) { this.dispose(); return; }
...
function useAutoMusic(style) {
  ...
  useEffect(() => {
    if (!style || style === "off") {
      musicEngine.token++;   // 作廢任何進行中的播放
      musicEngine.dispose(); // 無條件立刻停止
      return;
    }
