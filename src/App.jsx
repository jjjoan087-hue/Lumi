...
const musicEngine = {
  ...
  _teardown() {
    // 立刻切斷總音量，聲音瞬間消失（即使個別節點還沒 dispose）
    if (this.master) {
      try { this.master.gain.cancelScheduledValues(0); } catch {}
      try { this.master.gain.setValueAtTime(0, 0); } catch {}
      try { this.master.gain.value = 0; } catch {}
      try { this.master.disconnect(); } catch {}
    }
    // 全域輸出瞬間靜音，連殘響／長尾音也一起切掉
    try { Tone.getDestination().mute = true; } catch {}
    // 停掉每個 loop / 發聲節點
    ...
    if (this.master) { try { this.master.dispose(); } catch {} }
    this.nodes = [];
    this.master = null;
    this.current = null;
  },
  ...
  async _run(style, myGen) {
    ...
    // 這一軌所有聲音都先進 master，再到喇叭；關閉時切斷 master 即全部靜音
    try { Tone.getDestination().mute = false; } catch {}
    const master = new Tone.Gain(1).toDestination();
    this.master = master;
    const reverb = new Tone.Reverb({ decay: 6, wet: 0.5 }).connect(master);
...
function useAutoMusic(style) {
  ...
  // 掛載時先強制清空任何殘留的音軌（PWA 從背景喚醒時，舊的 loop 可能還在跑）
  useEffect(() => {
    musicEngine._teardown();
  }, []);
  useEffect(() => {
    const unlock = () => {
...
export default function App() {
  ...
  // 音樂固定為柔和鋼琴；musicStyle 只用來記住開("piano")或關("off")
  const musicStyle = settings.musicStyle === "off" ? "off" : "piano";
  useAutoMusic(musicStyle);
  const musicOn = musicStyle !== "off";
  const toggleMusic = () => {
    const next = musicOn ? "off" : "piano";
    setSettings((s) => ({ ...s, musicStyle: next }));
    if (next === "off") {
      // 立刻靜音，不必等佇列；同時作廢所有排隊中的播放
      musicEngine.gen++;
      musicEngine._teardown();
    } else {
      // 按鈕本身就是使用者手勢 → 可直接開始
      musicEngine.play("piano");
    }
  };
