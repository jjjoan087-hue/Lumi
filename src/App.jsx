...
function GlobalStyle() {
  return (
    <style>{`
      ...
      /* 手機橫向時，蓋一層提示請轉回直向（App 為直向設計） */
      #rotate-guard { display: none; }
      @media (max-width: 900px) and (orientation: landscape) {
        #rotate-guard {
          display: flex; position: fixed; inset: 0; z-index: 9999;
          background: #FEFCF7; color: #4A4638;
...
export default function App() {
  ...
  return (
    <div id="app-root" className="fixed inset-0 flex flex-col app-bg-gradient text-main font-sans overflow-hidden">
      <GlobalStyle />
      <div id="rotate-guard">
        <RotateCw size={30} className="text-mute2" />
        <div className="font-serif text-lg">請轉回直向使用</div>
        <div className="fs-12 text-mute">Lumi 是為直向畫面設計的</div>
      </div>
  ...
  return (
    <div className="relative">
      ...
      <div>
        ...
        <div ref={listRef} className="border border-line rounded-2xl divide-y divide-line bg-paper">
          {todos.length === 0 && (
          ...
          {todos.map((t, idx) => (
            ...
            >
              <span
                ...
                className="text-line2 shrink-0 touch-none cursor-grab"
